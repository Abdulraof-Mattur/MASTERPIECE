...
function CreateAccount() {
  ...
  const validateFields = () => {
    ...
    if (!passwordPattern.test(fields.password)) {
      newErrors.password =
        "Password must be 6-20 characters long and contain at least one uppercase letter, one lowercase letter, and one digit";
      isValid = false;
    }
  ...
  const fetchCreateAccount = async () => {
    try {
      const response = await axios.post(
        "http://localhost:8000/api/authAdmin/createAccount",
        fields
      );
